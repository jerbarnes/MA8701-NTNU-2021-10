...
if __name__ == "__main__":
    ...
    training_epochs = 10
    training_data = "data/train_medium.tsv"

    # Load the data and create the vocabulary at the same time
    # The Vocab object creates a new entry for each new word
    # that it sees while loading the datasets
    vocab = Vocab()
    train_data = SentimentDataset(training_data, vocab).get_split()
    ...
    train_y, train_text = zip(*[l.strip().split("\t") for l in open(training_data)])
    ...
    for i, epoch in enumerate(range(training_epochs)):
        model.train()
        ...
        for texts, labels in train_loader:
            ...
            optimizer.step()
            model.zero_grad()
        print("Loss: {0:.3f}".format(loss / training_epochs), end="\t")
